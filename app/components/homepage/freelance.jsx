...
export default function FreelanceSection() {
  ...
  const handleButtonClick = (type, url, isMail = false) => {
      ...
      <div className="flex flex-col lg:flex-row items-center justify-between max-w-6xl mx-auto px-2 py-2 gap-2 w-full flex-1">
        ...
        <div className="w-full lg:w-[48%] bg-white/10 backdrop-blur-md border border-[#25213b] rounded-2xl shadow-xl p-3 md:p-5 flex flex-col items-center gap-3">
          ...
          <div className="mt-2 w-full flex justify-center">
            <button
              onClick={() => handleButtonClick('quote', `mailto:${personalData.email}?subject=Let's%20Start%20Your%20Project`, true)}
              disabled={loading.quote}
              className="inline-block bg-gradient-to-r from-pink-500 to-violet-600 hover:from-violet-600 hover:to-pink-500 text-white font-bold py-2 px-6 rounded-full text-xs md:text-base shadow-lg transition animate-pulse disabled:opacity-60 disabled:cursor-not-allowed"
            >
              {loading.quote ? <Spinner /> : "Get a Free Quote Now"}
            </button>
          </div>
        </div>
      </div>
